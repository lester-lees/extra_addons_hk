...
class loewieec_shop(osv.osv):
    ...
    def create_shipping_address(self, cr, uid, partner_id, vals, context=None):
        ...
        if 	vals.get('state', False) :	
            state_id = self.pool.get('res.country.state').search(cr,uid,[('name','=ilike',vals.get('state'))])
    ...
    def create_order(self, cr, uid, ids, trade, context=None):	
        ...
        tids = []		
        for order in trade:		
            if order['is_kevin'] or order['is_sz'] : # 刷单及深圳发货订单不录入HK-ERP
                continue
        ...
        if len(order_val['order_line']) < 1: 
            ...
            return False
            #raise osv.except_osv(u'TMall后台无有效订单',u'''TMall后台有效订单已全部导入，或修改订单时段后再导.''')		
			
        order_id = order_obj.create(cr, uid, order_val, context = context)
        history_log = shop.last_log or ""
        str_tmp = u"时间：%s 共导入：%d 个天猫单，单号如下："  % ( self.get_datetime_now(), len(tids))	
    ...
    def import_coe_no(self, cr, uid, ids, context=None):
        ...
        while row_start <= highest_row and ws.cell(row=row_start,column=0).value :		
            ...
            coe_lines[tmijdi_no] = {"tmi_jdi_no": tmijdi_no, "name":coe_no, "receive_name":receive_name, "tel":tel, "address":address, "city":city, "state":state, "zip":zip, "sale_id":sale_id}				
